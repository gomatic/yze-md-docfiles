...
const helperEnabled = "DOCFILES_IGNORE_HELPER"
...
func stubGit(t *testing.T, stdout string, code int) {
	...
	execCommand = func(name string, args ...string) *exec.Cmd {
		...
		command.Env = append(os.Environ(),
			helperEnabled+"=1",
			"DOCFILES_IGNORE_OUT="+stdout,
			"DOCFILES_IGNORE_CODE="+strconv.Itoa(code),
		)
		return command
	}
...
func TestGitCheckIgnoreTreatsNothingIgnoredAsSuccess(t *testing.T) {
	...
	ignored, err := gitCheckIgnore(".", []string{"docs/README.md"})
...
func TestGitIsAskedTheRightQuestionFromTheRightPlace(t *testing.T) {
	...
	_, err := gitCheckIgnore(repoDir(root), []string{"a.md"})
...
func TestEachRepositoryIsAskedSeparately(t *testing.T) {
	...
	kept := tracked(ignores, []string{"/a/keep.md", "/a/ignored.md", "/b/keep.md", "/b/ignored.md"})

	assert.Equal(t, []string{"/a/keep.md", "/b/keep.md"}, kept)
...
func TestMarkIgnoredMustNotDisableTheFilterForOtherRepositories(t *testing.T) {
	...
	ignores := func(root repoDir, _ []string) (map[string]bool, error) {
		...
		return map[string]bool{"/b/ignored.md": true}, nil
	}

	kept := tracked(ignores, []string{"/a/ignored.md", "/b/ignored.md", "/b/keep.md"})

	assert.Equal(t, []string{"/a/ignored.md", "/b/keep.md"}, kept,
